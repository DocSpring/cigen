...
/// Render CI configs from cigen.yml using the plugin-based orchestrator
pub fn render_command(file: Option<String>, output: Option<String>) -> Result<()> {
    // Find cigen.yml
...
fn find_cigen_yml(file: Option<String>) -> Result<PathBuf> {
    ...
    anyhow::bail!(
        "No cigen.yml found. Tried: cigen.yml, .cigen/cigen.yml, cigen.yaml, .cigen/cigen.yaml"
    )
}
