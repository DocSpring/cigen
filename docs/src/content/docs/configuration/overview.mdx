...
workflows:
  main:
    jobs:
      - test`} lang="yaml" title="Basic cigen.yml" />
...
<Aside type="tip">
Notice how cigen uses `name` and `run` at the same level, reducing nesting and improving readability.
</Aside>
...
steps:
  - restore_cache:
      keys:
        - v1-gems-{{ checksum "Gemfile.lock" }}-{{ checksum ".ruby-version" }}
  - run: bundle install
  - save_cache:
...
<Code code={`jobs:
  build_image:
    ...
    steps:
      - run: docker build -t myapp:latest .`} lang="yaml" title="Multi-architecture builds" />
...
<Aside type="note">
Workflows are automatically discovered from directory structure - no need to maintain separate workflow definitions.
</Aside>
...
- Learn about [job configuration](/configuration/jobs/)
- Explore [workflow definitions](/configuration/workflows/)
- Understand the [cache system](/configuration/cache/)
- Read about [template engine](/configuration/templating/) features
