...
<Aside type="tip">
Job skipping only occurs after successful completion - failed jobs never create skip markers, ensuring reliability.
</Aside>
...
source_file_groups:
  ...
  docker:
    - 'Dockerfile*'
    - '.dockerignore'
    - 'docker-compose*.yml'`} lang="yaml" title="Source file group definitions" />
...
<Code code={`jobs:
  ...
  frontend_test:
    image: cimg/node:18
    source_files:
      - '@frontend'        # Named group
      - 'jest.config.js'   # Additional files
      - 'tsconfig.json'
    steps:
      - run: npm test`} lang="yaml" title="Using source file groups" />
...
<Code code={`jobs:
  shellcheck:
    ...
    source_files:
      ...
      - '.shellcheckrc'
    steps:
      - run: shellcheck scripts/*.sh`} lang="yaml" title="Inline source file patterns" />

## Automatic Template Inclusion

<Aside type="note">
**Automatic Behavior**: Cigen automatically includes relevant CI template files in source file hashes, so template changes trigger job re-runs.
</Aside>
...
<Code code={`jobs:
  build_multiarch:
    ...
    source_files: '@docker'
    steps:
      - run: docker build -t myapp:latest .`} lang="yaml" title="Architecture-aware skipping" />
...
<Code code={`- run:
    ...
    command: |
      ...
      fi`} lang="bash" title="Skip check logic" />
...
<Code code={`- run:
    ...
    when: on_success  # Only run if job succeeds`} lang="bash" title="Success recording" />
...
<CardGrid>
  <Card title="Time Savings" icon="rocket">
    Skip jobs when source files haven't changed - often 50-80% reduction in CI time
  </Card>
...
<Aside type="caution">
**Be Precise**: Only include files that actually affect the job outcome. Too broad patterns reduce skip efficiency.
</Aside>
...
# Bad - too broad
source_files:
  - '**/*'  # Includes everything - rarely skips
...
<Code code={`jobs:
  ...
  lint_javascript:
    source_files:
      ...
      - 'tsconfig.json'   # TypeScript config`} lang="yaml" title="Complete dependencies" />
...
<Steps>
1. **Check patterns match files**: Verify glob patterns find expected files
2. **Remove broad patterns**: Avoid `**/*` or overly inclusive patterns
3. **Check for changing files**: Look for files that change on every run (logs, timestamps)
4. **Validate template stability**: Ensure CI templates aren't changing
</Steps>
...
<Steps>
1. **Expand source files**: Add missing dependencies and config files
2. **Check ignored files**: Ensure important files aren't in `.gitignore`
3. **Clear skip cache**: Remove `/tmp/cigen_skip_cache` to reset state
4. **Add debug logging**: Use verbose mode to see hash calculations
</Steps>

### Debug Hash Calculation

<Code code={`# Add to job for debugging
steps:
  - run:
      ...
      command: |
        ...
        # Show exactly which files contribute to hash`} lang="bash" title="Debug source file tracking" />
...
<Aside type="note">
Skip rates depend on development patterns. Feature branches touching specific components see the highest skip rates.
</Aside>
