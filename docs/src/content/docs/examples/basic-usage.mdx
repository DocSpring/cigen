...
<Code code={`  $schema: https://cigen.dev/schemas/v1/config-schema.json
  ...
  workflows:
    main:
      jobs:
        - test`} lang="yaml" title=".cigen/config.yml" />
...
- Explore [advanced features](/cigen/advanced/multi-arch/) like OR dependencies
- Learn about [CircleCI-specific features](/cigen/providers/circleci/)
- Read about [intelligent caching](/cigen/configuration/cache/)
- Understand [job skipping](/cigen/advanced/job-skipping/) for maximum efficiency
